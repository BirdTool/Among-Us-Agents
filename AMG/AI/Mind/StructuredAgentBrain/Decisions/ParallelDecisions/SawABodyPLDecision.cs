...
namespace AMG.AI.Mind.StructuredAgentBrain.Decisions.ParallelDecisions
{
    internal class SawABodyPLDecision : IParallelDecision
    {
        ...
        private void SawABodyAction(List<RoundDeadBody> bodies, StructuredAgentBrain brain)
        {
            ...
            if (!Utils.ExecuteProbability(shouldLookAround))
            {
                ...
                {
                    LogManager.LogError($"[Agente {brain.Agent.PlayerId}] Tentou reportar corpo do ID {mostRecentBody.PlayerId}, mas o registro não existe mais!");
                }
            ...
            else
            {
                ...
                Waypoint agentNodeStart = Pathfinder.GetClosestNode(brain.Agent.transform.position);

                Vector2 agentPos2D = brain.Agent.transform.position;
